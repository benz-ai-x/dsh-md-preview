...
const result = { version, sourceCommit: '18b43c0810237c94bedb9200d05b496991e687dc', node: process.version, pnpm: execFileSync('pnpm', ['--version'], { encoding: 'utf8' }).trim(), baseline: '0.1.5-alpha.1+a28c5a8f4927217f345d02e22c782a32d5750f0a', profile, archiveSha256: digest(readFileSync(archive)), shutdown: [] }
writeFileSync('/tmp/mdpreview-0110a5-smoke-home.txt', smokeHome + '\n')
...
async function http(run, port, priorUrl) {
  ...
  for (const marker of [`v${version}`, 'beforeClose', 'dsh-md-tab', 'headingSource', 'mdSourceLine', 'Discard changes', '原生文本']) assert.ok(content.includes(marker), marker)
  assert.ok(!content.includes('dsh-md-preview-panelglyph'))
  assert.ok(!content.includes('md-preview-panel'))
  const assets = [...html.matchAll(/(?:src|href)="([^"\s]+\.js)"/g)].map(match => match[1])
  let headingSourceAsset
  for (const url of [...new Set(assets)]) {
    const assetResponse = await fetch(new URL(url, base), { headers })
    assert.equal(assetResponse.status, 200)
    const asset = await assetResponse.text()
    if (asset.includes('data-md-source-line') && asset.includes('data-md-source-offset')) {
      headingSourceAsset = { url, sha256: digest(asset) }
      break
    }
  }
  assert.ok(headingSourceAsset, 'Served platform does not contain heading source positions')
  return { status: response.status, entries: boot.entries.length, bytes: Buffer.byteLength(content), servedSha256: digest(content), url: entry.url, releaseMarkers: true, headingSourceAsset }
}
...
try {
  ...
  const override = join(smokeHome, 'markdown-override.yml')
  writeFileSync(override, '- id: md-preview\n  config:\n    maxBytes: 4096\n    allowedExtensions: [".md"]\n    previewExtensions: [".markdown"]\n    searchMaxResults: 3\n    searchMaxDirectories: 4\n    searchConcurrency: 2\n')
  const combined = await command(['--profile', 'web', '--patch', override, '--dump-config'], 'dump-overridden')
  const markdownConfig = combined.slice(combined.lastIndexOf('- id: md-preview'))
  for (const expected of ['maxBytes: 4096', 'allowedExtensions:\n      - .md', 'previewExtensions:\n      - .markdown', 'searchMaxResults: 3', 'searchMaxDirectories: 4', 'searchConcurrency: 2']) {
    assert.ok(markdownConfig.includes(expected), `Missing config override: ${expected}`)
  }
  result.configuration = { composed: true, override: { maxBytes: 4096, allowedExtensions: ['.md'], previewExtensions: ['.markdown'], searchMaxResults: 3, searchMaxDirectories: 4, searchConcurrency: 2 } }
  ...
  await stop(server)
  server = null
  writeFileSync('/tmp/mdpreview-0110a5-packed-smoke.json', JSON.stringify(result, null, 2) + '\n')
