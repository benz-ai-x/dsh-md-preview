...
`feat/official-sidebar-markdown` 的插件 `0.11.0-alpha.5` 迁移候选使用官方右栏；
固定 Harness 补丁提交 `a28c5a8f4927217f345d02e22c782a32d5750f0a`。
`DSH_HARNESS_ROOT=/Users/pc2026/DSH-Space/deepseek-harness-md-guard`，原检出和用户实例未修改。
用户要求最新 Harness 后重新核对官方发布、远程 master 与原检出，均为
`0.1.5-alpha.1` / `5dda764`；npm `alpha` 为该版，`latest` 标签仍是旧的 `0.1.2-rc.1`。
官方 npm 的 0.1.5-alpha.1 尚无公共关闭守卫与标题源位置选项，需依次应用 README 链接的两份补丁。
PR #61 前轮问题及本轮标题定位 P2 已修复，当前自动化201项通过。
插件 alpha.5 的同归档安装/移除及实际验收边界见
[标题定位修复](verification/official-sidebar-markdown/PR61-HEADINGS.md)；
alpha.4 证据保留于 [后续修复记录](verification/official-sidebar-markdown/PR61-FOLLOWUP.md)，不沿用为新包结果。
